...
- **Target management**: `ccdash target add <name>`, `ccdash target remove <name>`, `ccdash target list`, `ccdash target use <name>`
- **Authentication**: `ccdash target login <name>`, `ccdash target logout <name>`, `ccdash target check`
- **Server diagnostics**: `ccdash doctor` (connectivity check), `ccdash version` (CLI version info)
...
# Add a remote server
ccdash target add staging http://staging-ccdash.company.com
...
Authentication is stored per-target:
...
Confirm the CCDash server is running. For local dev, start it with `npm run dev`. Check the target URL with `ccdash doctor`.

**Project not resolved:**
Pass `--project <id>` to override the active project, or confirm the configured project exists.

**Authentication error:**
Log in with `ccdash target login <target>` and verify credentials with `ccdash target check`.
