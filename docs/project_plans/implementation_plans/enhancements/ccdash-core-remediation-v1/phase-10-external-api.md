...
> ⚠ **Forward-compat with `remote-ccdash-streaming` auth (see plan §Contending / Unmerged Work).** The paused streaming branch's ADR-008 specifies a **workspace-scoped per-token** auth model (`(workspace_id, hashed_token)` table) layered on the existing bearer guard, intended to extend — not replace — it. The single `CCDASH_API_TOKEN` chosen here (OQ-6, T10-004) must be implemented as an **auth dependency** on the v1 routes that a future workspace-token resolver can supersede without rewriting handlers — i.e. resolve identity in one injectable place, not inline per route. Do not bake "single global token" assumptions into handler bodies or the capability endpoint.
