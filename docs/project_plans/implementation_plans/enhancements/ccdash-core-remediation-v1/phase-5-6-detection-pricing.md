...
> ⚠ **Contending unmerged work — `remote-ccdash-streaming` (see plan §Contending / Unmerged Work).**
> The paused `feat/remote-ccdash-streaming*` branches already refactored `sync_engine.py` behind a `SessionIngestSource` port (cursor table = single source of sync truth, ADR-009) and added a `source_ref` URI column to `sessions` with the upsert key changed to `(project_id, source_ref)`. That code is **not in this branch**, but to stay mergeable:
> - **Do NOT add a `source_ref` / `source_type` column or alter the session upsert key here.** The new detection columns (`workflow_id`, `subagent_parent_id`, `skill_name`, `context_window`, canonical-slug) are independent of that identity model and are fine to add.
> - **Keep the T5-003 sidecar→record join behaviorally additive and localized** in `sync_engine.py` — express it as a discrete step that could later move behind the ingest-source port; do not re-architect the source/cursor boundary.
