...
- **AAR review loop**: Persisted `aar_reviews` rollup (deterministic triage: 5 flags, 3 verdicts), `CCDASH_AAR_REVIEW_AUTONOMOUS_WORKER_ENABLED` flag + quota gates (`_ESCALATION_QUOTA`, `_ESCALATION_WINDOW_HOURS`, `_NEW_SKILL_THRESHOLD`, `_LOOKBACK_DAYS`), 3 self-recursion guards (provenance, dedup, quota). Default-off worker. No LLM on compute path. See `docs/guides/aar-review-loop.md` for operator guidance.
