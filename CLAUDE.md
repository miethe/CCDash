...
# Grant/revoke per-project hosted-LLM egress consent (exactly one flag required)
...
- **System-wide metrics**: `backend/application/services/agent_queries/system_metrics.py` is the extension point for cross-project live metrics. Add new system-wide rollups (sessions/day, token totals, error rates) here and surface them via REST/MCP/CLI per the transport-neutral pattern. Operator/test guidance: `.claude/worknotes/system-wide-metrics/feature-guide.md`.
- **Artifact intelligence exchange**: SkillMeat snapshot, ranking, recommendation, and rollup surfaces live across `backend/application/services/agent_queries/artifact_intelligence.py`, `backend/services/artifact_*`, and the worker exporter.
  Operator/test guidance: `.claude/worknotes/skillmeat-artifact-usage-intelligence-exchange-v1/feature-guide.md`.
...
- **Frontend data layer**: All server state in TanStack Query QueryClient; domain query hooks in `services/queries/` keyed via registry at `services/queryKeys.ts`; bundle endpoints in `backend/application/services/agent_queries/`. See `docs/guides/feature-surface-architecture.md` for cache tiers (server `@memoized_query` ~600s TTL + client TQ 30s–5min staleTime), hook APIs, and invalidation patterns.
...
- **Incremental link rebuild**: `CCDASH_INCREMENTAL_LINK_REBUILD_ENABLED` (default **true**) enables family-scoped incremental link rebuild on the watcher hot path. When true, a new/changed JSONL triggers `rebuild_links_for_entities` (scoped delete + re-derive, no filesystem walk) instead of the global `_rebuild_entity_links` scan. Proven in Phase 4 (CCDash Core Remediation). Set `false`/`0` to revert to full-scan fallback.
...
- **Session redaction**: `CCDASH_REDACTION_PATTERNS_ENABLED` (default true — Layer 1 known-secret pattern scan) and `CCDASH_REDACTION_TOOL_AWARE_ENABLED` (default true — Layer 2 tool-name-aware field redaction) gate session detail egress in `backend/application/services/agent_queries/redaction.py`. Both default enabled (fail-closed); redaction-event logs emit field COUNT only, never payload contents. See `docs/guides/redaction-tuning.md`.
- **Sync coalescing guard**: `CCDASH_SYNC_COALESCING_ENABLED` (default true) gates a `(project_id, trigger)`-keyed in-process coalescing guard and durable-queue idempotent-enqueue — duplicate sync dispatches for the same key collapse to one in-flight run, never silently dropped. `CCDASH_SYNC_RECENT_FIRST_ENABLED` (default true) and `CCDASH_SYNC_RECENT_FIRST_N` (default 200) front-load the N most-recently-modified JSONL files each sync pass before backfilling the rest. See `docs/guides/sync-coalescing-recent-first.md`.
- **Launch-time capture**: `scripts/hooks/ccdash_capture_session_start.py` is a fail-open SessionStart hook (always exit 0) that reads `CCDASH_LAUNCHER`, `CCDASH_LAUNCH_PROFILE`, `CCDASH_LAUNCH_EFFORT`, and `CCDASH_LAUNCH_MODEL` and writes a `<session-id>.capture.json` sidecar (sidecar `schemaVersion` **2**; reader still accepts v1) co-located with the JSONL. DB columns `launcher`, `profile`, `effort_tier`, `model_variant` are nullable; unknown == null, never defaulted. See `docs/guides/launch-time-capture-convention.md`.
- **Effort-tier provenance (Gap 4)**: `sessions.effort_tier_source` (nullable, both backends, v44) records WHICH lane supplied `effort_tier` — they differ in trust: `launch_env` / `codex_payload_effort` / `codex_collaboration_mode` (authoritative) vs `claude_settings` (SessionStart snapshot, can be stale) vs `inherited_parent` (reserved for Gap 2, unused). Canonical vocabulary: `backend/parsers/effort_provenance.py`. Written iff `effort_tier` is; non-null tier + null source == row predates the column (no backfill). Consumers MUST treat unknown tokens as "unknown provenance", never hard-fail. Not yet consumed by `routing_rollup` (that is DI-4b).
- **Session columns — detection, pricing, capture**: The `sessions` table carries detection columns (`model_slug`, `workflow_id`, `subagent_parent_id`, `skill_name`, `context_window`), a pricing column (`pricing_model_source` — empty string signals unpriced state), and capture columns (`launcher`, `profile`, `effort_tier`, `effort_tier_source`, `model_variant`). All columns exist in both SQLite and Postgres DDL (`CREATE TABLE` + `_ensure_column` ALTERs); every new column requires dual DDL in the same change set plus a `COLUMN_PARITY_DRIFT_ALLOWLIST` check.
- **ICA key + spend capture (v51)**: `sessions.ica_key` + `ica_spend_start`/`ica_spend_end`/`ica_spend_delta`/`ica_spend_attribution` (all nullable TEXT, both backends). Sidecar `schemaVersion` **3** (reader still accepts v1/v2). `ica_key` is the ICA key NAME (CC1..CC6) from `CCDASH_LAUNCH_ICA_KEY` — NEVER secret bytes; unset stays NULL, **never defaulted to CC1**. Raw start/end come from the `x-litellm-key-spend` gateway response header via a strictly-gated fail-open 1-token probe on SessionStart / SessionEnd in `scripts/hooks/ccdash_capture_session_start.py` (`/key/info` and friends were probed 404 on the ICA gateway; the response header is the only read). `ica_spend_delta` is populated **only** for the `attributed` verdict; every other verdict stores NULL — *never silently divided* is structural, enforced by `decide_attribution` in `backend/parsers/ica_spend.py`. Closed attribution vocab: `attributed` / `concurrent_shared_key` / `key_changed` / `incomplete_readings` (consumers must treat unknown tokens as "unknown"). Delta+attribution derived post-upsert by `backfill_ica_spend_attribution` (sync-engine phase 1); session-detail exposure via `_apply_launch_capture` (snake→camelCase). AC4 fallback: `None` == "Not captured" in the API, never defaulted. Requires an out-of-repo launcher edit: `export CCDASH_LAUNCH_ICA_KEY="${ICA_KEY:-}"` in `~/ica-claude.sh` and a `SessionEnd` hook registration in `~/.claude/settings.json`.
- **Per-project egress consent write path**: `projects.llm_egress_consent` (v53) is the per-project half of the two-level fail-closed hosted-LLM egress gate (`CCDASH_LLM_EGRESS_CONSENT` is the other). The ONLY supported write path is `POST /api/projects/{project_id}/egress-consent` with body `{"granted": bool}` (`granted` required — an empty body is a 422, never an implicit revoke), surfaced as `ccdash-cli project consent <id> --grant|--revoke`. `DbProjectManager.update_project` carries a stored consent value forward whenever the incoming `Project` did not explicitly set the field (`model_fields_set`), so a partial update can never silently revoke it; `add_project` is deliberately NOT given that treatment, so a genuinely new project still defaults to consent FALSE. Grant procedure: `docs/guides/external-api-lan-deployment.md` § Granting the per-project half.
- **Session-detail + transcript endpoints**: `GET /api/v1/sessions/{id}/detail` returns the full transcript-bearing bundle for any project (`project_id` required); `GET /api/v1/sessions/{id}/transcript` returns a cursor-paginated transcript page (`{items,cursor,limit,nextCursor}` envelope). Source of truth: `backend/application/services/agent_queries/session_detail.py`. See `docs/guides/external-api-lan-deployment.md`.
- **Capability advertisement**: `GET /api/v1/capabilities` returns the server-declared capability set (e.g. `sessions:detail`, `sessions:cross-project`) for IntentTree and LAN agents. Consumers MUST NOT hard-fail on unknown capability strings — absent means the server predates that feature; present means the documented contract is honoured. See `docs/guides/external-api-lan-deployment.md`.
- **Remote session ingest**: `POST /api/v1/ingest/sessions` (NDJSON, bearer/workspace auth); local daemon `ccdash-cli daemon start|status|replay` (retry/backoff + dead-letter NDJSON at `~/.local/state/ccdash/deadletter/` + replay); workspace-scoped auth (`DEFAULT_WORKSPACE_ID="default-local"`, ADR-008); multi-project routing via `AuthContext.project_id` (ADR-010); `SessionIngestSource` port + `ingest_cursors` table (ADR-009). Health: `/api/health` `auth_mode`; `/api/health/detail` `ingest_sources[]` (per-source lag/state, `CCDASH_INGEST_SOURCE_FRESH_SECONDS`/`_STALE_SECONDS`). ADR-014 (transport), ADR-015 (daemon). Guides: `docs/guides/remote-ingest-operator-guide.md`, `docs/guides/remote-streaming-v1-to-v2-migration.md`.
- **IntentTree node ↔ session cost attribution**: `POST /api/v1/intent-nodes/{node_id}/sessions` idempotently declares node↔session bindings as `entity_links` rows (`source_type='intent_node'`, `origin='declared'` — no schema migration); `GET /api/v1/intent-nodes/{node_id}/cost` (`project_id` required) rolls up `tokens_in`/`tokens_out`/`total_cost` from `sessions`, reporting `attributionScope: "declared" | "family"` (opt-in `?expand_family=true` widens to each declared session's `workflow_id` family — the caller owns the wider claim). Service: `backend/application/services/agent_queries/intent_node_cost.py`; repository: `backend/db/repositories/entity_graph.py`'s `INTENT_NODE_LINK_*` block. `GET /api/v1/sessions/{session_id}/tool-calls` (`project_id` required, optional `tool` exact-match filter) makes `session_logs` rows reachable externally without direct postgres access, reusing `SessionTranscriptService.list_session_logs` in the same `{items,cursor,limit,nextCursor}` envelope as `/transcript`. Capabilities: `intent-nodes:cost`, `sessions:tool-calls`. See `docs/guides/external-api-lan-deployment.md`.
- **Watcher fan-out**: Registry-driven per ADR-006; `CCDASH_WORKER_WATCH_PROJECT_ID` optional scope filter (empty/unset → all registered projects). Tunable: `CCDASH_WATCHER_SYNC_CONCURRENCY` (default 20), `CCDASH_WATCHER_RECONCILE_INTERVAL_SECONDS` (default 60). Postgres in-place v29→v35 upgrade covered by `npm run docker:hosted:smoke:seeded-pg`.
- **Watcher readiness (`/readyz`) — zero watch paths**: On a **required-watcher** profile (`worker-watch`), the watcher readiness verdict is derived from the per-project map in `backend/adapters/jobs/runtime.py:_watcher_probe_detail`, which is populated for EVERY registered project — including single-project env-pin mode (`fanOutMode: false`). Rules, in `backend/runtime/container.py`: any project resolving real paths → that set alone gates readiness, so one `configured_no_paths` project **alongside** watchable ones only **warns** (resilience-by-default — a pathless registry entry must not poison a worker that is correctly watching every real path); **every** bound project `configured_no_paths` **and** the watcher required → **`fail` / HTTP 503 `configured_no_paths`** (there is genuinely nothing to watch). Optional-watcher profiles (`local`) stay `warn`. Watch paths are existence-filtered (`file_watcher.py:_resolve_watch_paths`), so a registered-but-absent directory contributes zero. The all-pathless hard-fail was previously unreachable dead code — the per-project branch always won; restored 2026-08-11 (PR #63). Regression guards: `test_worker_watch_readyz_fails_when_every_bound_project_has_no_watch_paths` and `test_worker_watch_readyz_stays_ready_with_one_pathless_project_beside_a_running_one` in `backend/tests/test_runtime_bootstrap.py`. CI: the `smoke-no-paths` job in `.github/workflows/enterprise-e2e-smoke.yml` asserts the 503. Because `/readyz` gates `compose up --wait`, `/redeploy` health gates and orchestrators, **never start a stack with `--wait` on a service you are deliberately misconfiguring** — wait on its dependencies instead.
- **Offline CLI mode** (repo-local only): `--offline` flag (or `CCDASH_OFFLINE=1`) on `backend/.venv/bin/ccdash` / `python -m backend.cli` bootstraps a local cache DB (`~/.ccdash/offline-cache.db`), runs a scoped read-only `SyncEngine.sync_project(allow_writeback=False)` against the project's raw JSONL, then executes the same query stack against the seeded DB — no server or worker needed. Additional flags: `--ephemeral` (temp-file DB, discarded on exit), `--refresh` (force full re-parse), `--config <path>` (offline registry override). The offline registry reuses the `projects.json` export shape; resolution precedence: `--config` → `CCDASH_PROJECTS_FILE` → `~/.ccdash/projects.json` → `./projects.json`. Worker-only enrichments (cost, analytics KPIs, context observability, intelligence facts) degrade to zero/null — contract state, not a bug; a stderr banner makes this explicit. Standalone `ccdash-cli` (pipx) is HTTP-only and does not support offline mode. See `docs/guides/offline-cli.md` and `backend/cli/offline.py`.
- **Persona extract** (repo-local only, P4 of the universal persona memory system): `ccdash persona extract` mines a single Claude Code session JSONL post-session and appends high-signal persona-candidate lines to the universal persona memory bank's `_inbox/capture.jsonl`. **No model calls, no DB writes, never opens the cache DB** — it reads one JSONL by path and writes only to files under `${OP_PERSONA_HOME:-~/.claude/memory}` (`_inbox/capture.jsonl` + `_meta/ccdash-extract-state.json`). Strictly per-session: `--session <id>` xor `--latest` (default) xor `--since <iso>` (hard-capped at N≤25); `--all` is rejected. Deterministic regex rules (R1–R8) over user-message text only live in `backend/application/services/agent_queries/persona_extract_rules.py`; the pure `extract_candidates` service is in `persona_extract.py` (no IO); CLI plumbing (flock-guarded append, atomic state watermark, idempotency) is in `backend/cli/commands/persona.py`. The emitted candidate-line JSON is a **frozen byte-for-byte interop contract** with the agentic_meta_dev persona bank (see PRD §5); the shape is locked by `backend/tests/test_persona_extract_log_shape.py`. `--dry-run` and `--json` supported; `ccdash persona extract status` shows the state file. CCDash is the **producer**; agentic_meta_dev/operator_core owns reconciliation, model calls, and writeback.
- **AAR review loop**: Persisted `aar_reviews` rollup (deterministic triage: 5 flags, 3 verdicts), `CCDASH_AAR_REVIEW_AUTONOMOUS_WORKER_ENABLED` flag (default **true**) + quota gates (`_ESCALATION_QUOTA`, `_ESCALATION_WINDOW_HOURS`, `_NEW_SKILL_THRESHOLD`, `_LOOKBACK_DAYS`), 3 self-recursion guards (provenance, dedup, quota). Worker/worker-watch-profile-gated; sweeps EVERY registered project per tick (`AARReviewSweepJob._resolve_projects_to_sweep` → `workspace_registry.list_projects()`, ADR-006) — never scoped to the sync engine's single-project binding. No LLM on compute path. See `docs/guides/aar-review-loop.md` for operator guidance.
