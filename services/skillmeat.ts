...
export async function validateSkillMeatConfig(
  ...
  return apiRequestJson<SkillMeatConfigValidationResponse>(`${API_BASE}/validate-config`, {
    method: 'POST',
...
export async function syncSkillMeatDefinitions(projectId: string): Promise<SkillMeatDefinitionSyncResponse> {
  return apiRequestJson<SkillMeatDefinitionSyncResponse>(`${API_BASE}/sync`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function refreshSkillMeatCache(projectId: string): Promise<SkillMeatRefreshResponse> {
  return apiRequestJson<SkillMeatRefreshResponse>(`${API_BASE}/refresh`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ projectId }),
  });
...
export async function backfillSkillMeatObservations(
  ...
  return apiRequestJson<SkillMeatObservationBackfillResponse>(`${API_BASE}/observations/backfill`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      projectId,
