...
describe('getProjectPlanningSummary', () => {
  it('calls the correct URL without projectId', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/agent/planning/summary', expect.objectContaining({ credentials: 'same-origin' }));
  });
...
describe('getProjectPlanningGraph', () => {
  ...
  it('calls the correct URL without options', async () => {
    ...
    expect(vi.mocked(fetch)).toHaveBeenCalledWith('/api/agent/planning/graph', expect.objectContaining({ credentials: 'same-origin' }));
  });
...
describe('getFeaturePlanningContext', () => {
  ...
  it('calls the correct URL for a feature', async () => {
    ...
    expect(vi.mocked(fetch)).toHaveBeenCalledWith('/api/agent/planning/features/feat-1', expect.objectContaining({ credentials: 'same-origin' }));
  });
