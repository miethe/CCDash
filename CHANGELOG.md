...
- **Automated AAR Review Loop** — A model-free deterministic triage pipeline for After-Action Review documents. The read/query surfaces (REST/CLI/MCP) are production-ready; the autonomous worker is default-off and the escalation writeback seam is staged but unwired (see caveats below).
  ...
  - **Transport-neutral read surfaces**: `GET /api/v1/project/aar-review` LAN endpoint, `ccdash report aar-review` CLI command, and `ccdash_aar_review` MCP tool; new capability string `aar-review` advertised via `GET /api/v1/capabilities`.
  - **Autonomous worker with provenance safeguards**: Background job `CCDASH_AAR_REVIEW_AUTONOMOUS_WORKER_ENABLED` (default `false`) with three self-recursion guards — provenance self-exclusion (fail-closed: never re-review output from prior AAR review runs), dedup ledger (idempotent on the `(aar_document_id, session_id)` composite primary key), and per-project escalation quota (`CCDASH_AAR_ESCALATION_QUOTA` / `CCDASH_AAR_ESCALATION_WINDOW_HOURS`) — producing read-only observability logs (no model calls, no LLM-backed writeback; op-gated integration deferred).
