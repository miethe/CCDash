...
export const PlanCatalog: React.FC = () => {
  ...
  const activeDoc = activeFilePath ? MOCK_DOCUMENTS.find(d => d.filePath === activeFilePath) : null;
                        ...
                        {filteredDocs.map(doc => (
                            <div key={doc.id} onClick={() => setSelectedDoc(doc)} className="grid grid-cols-12 gap-4 p-4 border-b border-slate-800 hover:bg-slate-800/30 cursor-pointer transition-colors items-center text-sm">
                                ...
                                <div className="col-span-2">
                                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${
                                        doc.status === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-700 text-slate-400'
                                    }`}>{doc.status}</span>
                                </div>
                        ...
                        {activeDoc ? (
                            <div className="p-4 space-y-6">
                                <div>
                                    ...
                                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${
                                        activeDoc.status === 'active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-700 text-slate-400'
