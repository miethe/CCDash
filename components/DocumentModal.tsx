...
export const DocumentModal = ({ doc, onClose }: { doc: PlanDocument; onClose: () => void }) => {
  ...
  const linkedTasks = MOCK_TASKS.filter(t => doc.frontmatter.linkedFeatures?.includes(t.id));
  const linkedSessions = MOCK_SESSIONS.filter(s => doc.frontmatter.linkedSessions?.includes(s.id));
