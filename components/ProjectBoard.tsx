...
const ProjectStructureTree = ({ task, onOpenFile }: { task: ProjectTask; onOpenFile: (path: string) => void }) => {
  ...
  const { prds, plans, phases, context } = useMemo(() => {
    ...
    (task.relatedFiles || []).forEach(file => {
      const doc = documents.find(d => d.filePath === file);
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg">
            ...
            <div className={`font-bold capitalize ${task.priority === 'high' ? 'text-rose-400' : 'text-blue-400'
              }`}>{task.priority}</div>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg">
      ...
      <div className="w-1/3 border-r border-slate-800 pr-4 flex flex-col gap-2">
        ...
        {task.relatedFiles?.map(file => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors text-left truncate ${selectedFile === file
...
const TaskModal = ({ task, onClose }: { task: ProjectTask; onClose: () => void }) => {
  ...
  const linkedSessions = sessions.filter(s => s.taskId === task.id);
        ...
        <div className="p-6 border-b border-slate-800 flex justify-between items-start bg-slate-900">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded ${task.status === 'done' ? 'bg-emerald-500/10 text-emerald-500' :
      ...
      <div className="flex justify-between items-start mb-3">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${task.status === 'done' ? 'bg-emerald-500/10 text-emerald-500' :
                task.status === 'in-progress' ? 'bg-indigo-500/10 text-indigo-500' :
                  task.status === 'review' ? 'bg-amber-500/10 text-amber-500' :
                    'bg-slate-500/10 text-slate-500'
              }`}>
