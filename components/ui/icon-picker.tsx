...
>(({
  ...
  const renderIcon = useCallback((icon: IconData) => (
    <TooltipProvider key={icon.name}>
      <Tooltip>
        <TooltipTrigger
          className={cn(
            "bg-background p-2 rounded-md border border-border hover:bg-accent transition",
  ...
  return (
    <Popover open={open ?? isOpen} onOpenChange={handleOpenChange} modal={modal}>
      ...
      <PopoverContent className="w-64 p-2 bg-popover text-popover-foreground border border-border shadow-xl opacity-100">
        {searchable && (
          <Input
        ...
        {categorized && search.trim() === "" && (
          <div className="flex flex-row gap-1 mt-2 overflow-x-auto pb-2 bg-popover">
            {categoryButtons}
          </div>
        )}
        <div
          ref={parentRef}
          className="max-h-60 overflow-auto rounded-md bg-popover"
