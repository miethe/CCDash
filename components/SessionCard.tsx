...
// ── automatic-session-naming: sessionName provenance badge ────────────────────
// Named risk (PRD §9): a derived/generated name could be mistaken for a
// provider-set one. This surfaces the provenance token (session_name_source,
// backend/parsers/session_name_provenance.py) compactly next to any rendered
// session name. Unrecognised tokens are treated as "unknown provenance", never
// a crash — same closed-vocabulary-with-fallback contract as the source module.
export type SessionNameSource = string | null | undefined;

interface SessionNameProvenanceInfo {
  label: string;
  title: string;
  isProviderPersisted: boolean;
}

const SESSION_NAME_PROVENANCE_LABELS: Record<string, { label: string; title: string }> = {
  provider_persisted: {
    label: 'Provider',
    title: 'Session name set by the provider (Claude Code / Codex) — not derived by CCDash.',
  },
  derived_deterministic: {
    // NOT 'Inherited'. This one token covers four distinct derivations — parent-session
    // inheritance, a git branch, the session's last prompt, and a truncated first message
    // — and the last two are now the dominant population, so 'Inherited' described a
    // minority of the rows it labelled. Keep this in sync with the four cases listed in
    // backend/parsers/session_name_provenance.py's module docstring.
    label: 'Derived',
    title: 'Session name derived deterministically by CCDash (a parent session, a git branch, the last prompt, or the first message) — not provider-set.',
  },
  derived_embedding_transfer: {
    label: 'AI · Similar',
    title: 'Session name derived from a similar session by CCDash — best-effort, may not be fully accurate.',
  },
  derived_generative: {
    label: 'AI',
    title: 'Session name generated by CCDash — best-effort, may not be fully accurate.',
  },
  operator_set: {
    label: 'Manual',
    title: 'Session name set manually by an operator.',
  },
};

export const describeSessionNameProvenance = (source?: SessionNameSource): SessionNameProvenanceInfo | null => {
  const normalized = (source || '').trim();
  if (!normalized) return null;
  const known = SESSION_NAME_PROVENANCE_LABELS[normalized];
  if (known) return { ...known, isProviderPersisted: normalized === 'provider_persisted' };
  return {
    label: 'Unknown source',
    title: `Session name provenance token not recognised: "${normalized}".`,
    isProviderPersisted: false,
  };
};

/**
 * Compact badge surfacing sessionName provenance wherever a session name is
 * displayed. Renders nothing when there is no name to attribute (null/empty
 * sessionName) — this is a contract state, never a crash.
 */
export const SessionNameProvenanceBadge: React.FC<{
  sessionName?: string | null;
  sessionNameSource?: SessionNameSource;
  className?: string;
}> = ({ sessionName, sessionNameSource, className = '' }) => {
  if (!(sessionName || '').trim()) return null;
  const info = describeSessionNameProvenance(sessionNameSource);
  if (!info) return null;
  return (
    <span
      className={`text-[9px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded-full border ${
        info.isProviderPersisted
