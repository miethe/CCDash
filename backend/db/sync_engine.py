...
class SyncEngine:
    ...
    def _get_session_ingest_service(self) -> SessionIngestService:
        if getattr(self, "_session_ingest_service", None) is None:
            self._session_ingest_service = SessionIngestService(
                session_repo=self.session_repo,
                session_message_repo=self.session_message_repo,
                project_session_messages=lambda session_payload, logs: project_session_messages(
                    session_payload,
                    logs,
                ),
                apply_usage_fields=_apply_claude_usage_fields,
                should_write_legacy_session_logs=lambda canonical_rows: self._should_write_legacy_session_logs(
                    canonical_rows,
                ),
                derive_session_observability_fields=lambda project_id, session_payload, logs: (
                    self._derive_session_observability_fields(
                        project_id,
                        session_payload,
                        logs,
                    )
                ...
                maybe_enqueue_telemetry_export=lambda project_id, session_payload: (
                    self._maybe_enqueue_telemetry_export(
                        project_id,
                        session_payload,
                    )
                ),
                publish_transcript_appends=lambda session_payload, previous_logs, current_logs: (
                    _publish_session_transcript_appends(
                        session_payload,
                        previous_logs=previous_logs,
                        current_logs=current_logs,
                    )
                ),
                publish_session_snapshot=lambda session_payload, log_count, source: publish_session_snapshot(
                    session_payload,
                    log_count=log_count,
                    source=source,
                ),
            )
        return self._session_ingest_service
