"""Enterprise Postgres-backed StorageUnitOfWork adapter.

Implements the existing StorageUnitOfWork port using the same repository factory
helpers. In future phases this will move to explicit Postgres-owned repositories
and schemas per the modularization plan, but for Phase 2 it ensures that
composition is explicit and profile-driven without changing the port shape.
"""
...
class EnterpriseStorageUnitOfWork:
    ...
    # Port methods — identical surface to LocalStorageUnitOfWork
