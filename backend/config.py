...
# CCDASH_SNAPSHOT_FRESHNESS_MAX_AGE_SECONDS (default: 7 days)
# Uses the strictest OQ-5 snapshot freshness threshold for diagnostics health.
CCDASH_SNAPSHOT_FRESHNESS_MAX_AGE_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_MAX_AGE_SECONDS",
    7 * 24 * 60 * 60,
)
CCDASH_RANKING_MIN_SAMPLE_SIZE = _env_int("CCDASH_RANKING_MIN_SAMPLE_SIZE", 3)
CCDASH_RECOMMENDATION_MIN_CONFIDENCE = _env_float("CCDASH_RECOMMENDATION_MIN_CONFIDENCE", 0.6)
# ccdash-aar-review-mvp: deterministic AAR<->session triage thresholds. Both are
...
def resolve_session_naming_lane() -> str:
    """The ONE resolution of the effective session-naming lane name.

    Body is nothing but a :func:`resolve_with_legacy_fallback` call over the
    ``CCDASH_LLM_SESSION_NAMING_LANE`` (new, preferred) /
    ``CCDASH_SESSION_NAMING_BACKEND`` (legacy) pair, with ``"local"`` as the
    default -- see both attributes' comments above for the compatibility
    story, and ``resolve_with_legacy_fallback``'s docstring for precedence
    and the log-once deprecation warning.

    This exists as a FUNCTION, and every consumer calls it rather than
    re-deriving the pair, because the lane name has TWO consumers that must
    never disagree: ``resolve_naming_backend``
    (``backend/services/session_naming_local_backend.py``), which decides
    WHICH backend to construct, and ``SessionNamingSweepJob``'s per-tick
    egress AUDIT event (``backend/adapters/jobs/session_naming_sweep_job.py``),
    which REPORTS the lane that is egressing. They diverged once
    (hosted-llm-anthropic-ica-lane-v1 M3 reviewer gate: the audit line read
    the raw legacy attribute, so an operator who set only the new var got an
    egress audit line naming ``lane=local`` while the sweep egressed to ICA
    -- an audit line naming the wrong lane is worse than no audit line).
    Hoisting the var pair, the default, and the precedence into one callable
    makes that class of drift unrepresentable rather than merely fixed.

    Both attributes are read at CALL TIME as module globals (never captured
    into a third pre-computed constant), so patching EITHER one via
    ``patch.object(config, ...)`` -- which mutates exactly this module's
    global namespace -- is honored. That is the property the
    ``CCDASH_LLM_SESSION_NAMING_LANE`` comment above already committed to,
    and which the existing test suite (which patches the legacy attribute
    directly to drive the resolver) depends on.
    """
    return resolve_with_legacy_fallback(
        str(CCDASH_LLM_SESSION_NAMING_LANE or "").strip().lower(),
        str(CCDASH_SESSION_NAMING_BACKEND or "local").strip().lower(),
        "local",
...
CCDASH_SNAPSHOT_FRESHNESS_WORKFLOW_SPECIFIC_SWAP_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_WORKFLOW_SPECIFIC_SWAP_SECONDS",
    7 * 24 * 60 * 60,
)
CCDASH_SNAPSHOT_FRESHNESS_LOAD_ON_DEMAND_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_LOAD_ON_DEMAND_SECONDS",
    14 * 24 * 60 * 60,
)
CCDASH_SNAPSHOT_FRESHNESS_VERSION_REGRESSION_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_VERSION_REGRESSION_SECONDS",
    14 * 24 * 60 * 60,
)
CCDASH_SNAPSHOT_FRESHNESS_OPTIMIZATION_TARGET_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_OPTIMIZATION_TARGET_SECONDS",
    30 * 24 * 60 * 60,
)
CCDASH_SNAPSHOT_FRESHNESS_IDENTITY_RECONCILIATION_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_IDENTITY_RECONCILIATION_SECONDS",
    30 * 24 * 60 * 60,
)
CCDASH_SNAPSHOT_FRESHNESS_INSUFFICIENT_DATA_SECONDS = _env_int(
    "CCDASH_SNAPSHOT_FRESHNESS_INSUFFICIENT_DATA_SECONDS",
    30 * 24 * 60 * 60,
)
...
# ── are-we-winning-dashboard-v1 M1: IntentTree lifecycle-event ingestion ────
# All unset by default -- an unset CCDASH_INTENTTREE_API_URL makes the whole
# feature inert (no job constructed; see backend/runtime/container.py). Never
# hardcode a URL or token here; these are read from the environment only.
CCDASH_INTENTTREE_API_URL = os.getenv("CCDASH_INTENTTREE_API_URL", "").strip() or None
CCDASH_INTENTTREE_API_TOKEN = os.getenv("CCDASH_INTENTTREE_API_TOKEN", "").strip() or None
CCDASH_INTENTTREE_WORKSPACE_ID = os.getenv("CCDASH_INTENTTREE_WORKSPACE_ID", "").strip() or None
# Optional filter -- when unset, ingestion sweeps the whole workspace.
CCDASH_INTENTTREE_TREE_ID = os.getenv("CCDASH_INTENTTREE_TREE_ID", "").strip() or None
# Kill-switch for the ingestion job and (later) REST surface. Default False --
...
# single uncapped call actually returned everything.
CCDASH_INTENTTREE_INGEST_PAGE_SIZE = min(_env_int("CCDASH_INTENTTREE_INGEST_PAGE_SIZE", 200), 200)
...
CCDASH_ARTIFACT_ROLLUP_EXPORT_INTERVAL_SECONDS = _env_int("CCDASH_ARTIFACT_ROLLUP_EXPORT_INTERVAL_SECONDS", 3600)
CCDASH_LOCAL_USER_ROLLUP_SCOPE_MODE = os.getenv("CCDASH_LOCAL_USER_ROLLUP_SCOPE_MODE", "pseudonym").strip().lower()
CCDASH_LOCAL_USER_SCOPE_PSEUDONYM = os.getenv("CCDASH_LOCAL_USER_SCOPE_PSEUDONYM", "local-user").strip() or "local-user"
