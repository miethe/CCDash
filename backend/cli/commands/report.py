...
@report_app.command("aar-review")
def aar_review(
    document: str = typer.Option(..., "--document", "-d", help="AAR document id or path to triage."),
    output: OutputMode | None = typer.Option(None, "--output", help="Output format."),
    json_output: bool = typer.Option(False, "--json", help="Shortcut for --output json."),
    markdown_output: bool = typer.Option(False, "--md", help="Shortcut for --output markdown."),
) -> None:
    """Deterministic AAR-document-to-session triage review (no model calls)."""

    async def _query():
        async def _invoke(context, ports):
            result = await _aar_review_service.get_review(context, ports, document)
            return result, context.project.project_id if context.project else None

        return await runtime.execute_query(_invoke)

    try:
        result, project_id = runtime.run_async(_query())
    except Exception as exc:
        typer.echo(f"Error: {exc}", err=True)
        raise typer.Exit(code=1) from exc
    ...
    if result.status == "error":
        if not project_id:
            ...
            typer.echo(
                f"Error: {runtime.document_not_found_error_message(document, project_id)}",
                err=True,
            )
        raise typer.Exit(code=2)
    ...
    typer.echo(get_formatter(mode).render(result, title=f"AAR Review: {document}"))
