...
PLAN_SCALAR_KEYS = (
    ...
    "intenttree_tree",
    "doc_type",
    # M4 (operator-p0-state-integrity) addition: without `status` in this tuple no consumer
    # could ever compare plan-frontmatter status to bound-node status — the exact gap that let
    # `context-fabric-program-v1.md` (status: completed) and `rf-swarm-driver-and-litellm-v1.md`
    # (unresolvable tree slug) drift silently. Purely additive: every existing consumer either
    # destructures `PlanFile` by field name (unaffected by a new trailing field with a default)
    # or reads `scan_frontmatter_scalars()`'s dict via `.get()` (unaffected by an extra key).
    "status",
)
...
class PlanFile:
    ...
    status: str | None = None  # M4 addition — see PLAN_SCALAR_KEYS comment above
