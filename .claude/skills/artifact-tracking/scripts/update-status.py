...
# Frontmatter collections that carry `id`-bearing items a --task flag may target.
# `tasks` is searched first and always wins; the rest are non-task collections whose
# updates must NOT re-derive task metrics or the file-level status.
TASK_COLLECTION = 'tasks'
NON_TASK_COLLECTIONS = ('success_criteria', 'risks', 'open_questions')
ID_COLLECTIONS = (TASK_COLLECTION,) + NON_TASK_COLLECTIONS
...
def find_item(
    ...
    """
    Resolve an id against the id-bearing frontmatter collections.

    `tasks` is searched first so a task id always shadows an identically-named entry
    in another collection. First match wins; the search stops there.

    Args:
        frontmatter: Progress file frontmatter dictionary
        item_id: Identifier to resolve (e.g. "TASK-1.3", "SC-2")

    Returns:
        Tuple of (item dict, collection name), or (None, None) if unresolved.
    """
    for collection in ID_COLLECTIONS:
        items = frontmatter.get(collection)
...
def recalculate_metrics(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Only ever called for updates that resolved inside `tasks:` — a non-task update
    must not re-derive task counts or flip the file-level `status`.
    ...
    # `overall_progress` is the schema-REQUIRED completion field; keep the two in
    # lockstep so they cannot silently diverge (see schemas/VALIDATION-REFERENCE.md).
    frontmatter['overall_progress'] = progress
...
def update_task_status(
    ...
    # Get current progress (fall back to the schema-required field when `progress`
    # is absent, so the reported old→new figure is not a fabricated 0)
    old_progress = frontmatter.get('progress', frontmatter.get('overall_progress', 0))

    # Resolve the id — `tasks:` first, then the other id-bearing collections
    task, collection = find_item(frontmatter, task_id)

    if task is None:
        raise ValueError(
            f"Task '{task_id}' not found in {filepath} "
            f"(searched: {', '.join(ID_COLLECTIONS)})"
        )
    ...
    if collection == TASK_COLLECTION:
        # Recalculate metrics
        frontmatter = recalculate_metrics(frontmatter)
        new_progress = frontmatter.get('progress', 0)
    else:
        # A non-task update must not re-derive task metrics, and must never flip the
        # file-level `status` from an unrelated collection. Only the timestamp moves.
        frontmatter['updated'] = datetime.now().strftime('%Y-%m-%d')
