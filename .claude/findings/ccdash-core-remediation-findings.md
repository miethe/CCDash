...
## F-DEPLOY-002: Postgres in-place upgrade path is broken below SCHEMA_VERSION 35 (RESOLVED 2026-06-14)

**Status**: ✅ Resolved (W3 / Phase P1 + composite-FK follow-up, 2026-06-14). **Surfaced**: live-watch bring-up against a pre-existing PG volume, 2026-06-12. **Severity**: Medium (only affects existing Postgres DBs; fresh DBs unaffected).

**Resolution**: Implemented exactly the "Fix direction" below. (1) The `project_id`-dependent `CREATE INDEX` statements were moved out of `_TABLES` to run after the versioned column ALTERs, plus an unconditional `_ensure_column(sessions, project_id)` guards the remaining unconditional indexes (commits `442cfa2`, `3fe88f0`). (2) The deeper composite child-table FKs (`REFERENCES sessions(project_id, id)`) — which the container smoke surfaced and the unit mock missed — were moved out of `_TABLES` into the versioned `_migrate_v31` composite-PK migration with a bulk Step-0 pre-drop (commit `acfd626`). (3) Added the seeded-older-version smoke `npm run docker:hosted:smoke:seeded-pg` (boots a seeded v29 PG volume). Validated: real v29→v35 in-place upgrade reaches `migrationStatus="applied"` with `UndefinedColumnError` ABSENT; `backend/tests/test_postgres_migrations_upgrade.py` 26/26 green (guard-proven). Success metric met: PG in-place upgrade 0% → 100% for DB ≥ v29.
...
## F-DEPLOY-003: worker-watch binds exactly one hardcoded project id per process (RESOLVED 2026-06-14)

**Status**: ✅ Resolved (W2 / Phase P2 SPIKE + Phase P3 impl, 2026-06-14). **Surfaced**: operator critique, 2026-06-12. **Severity**: Medium (operability / multi-project UX).

**Resolution**: Implemented exactly the "Fix direction" below. The watcher now derives its target set from the DB-authoritative project registry (ADR-006): an empty/unset `CCDASH_WORKER_WATCH_PROJECT_ID` makes the worker-watch fan out across all registered projects in one process; a non-empty value is demoted to an optional scoping filter (backward-compatible). One `asyncio.Task` per project with supervisor isolation (one project's watch failure can't kill siblings) + a bounded sync semaphore (`CCDASH_WATCHER_SYNC_CONCURRENCY`), a per-project `/api/health/detail` watcher rollup, and a 60s registry reconcile loop (`CCDASH_WATCHER_RECONCILE_INTERVAL_SECONDS`). Commits `7c07c8d`, `0393063`, `4d4e4e6`, `9fe62d8`. Validated: `test_p3_worker_bootstrap.py` 33/33 + `test_p3_watcher_registry.py` 55/56 green. Full hot-reload signaling (sub-second push) is the remaining stretch, tracked as deferred spec D-002 (`docs/project_plans/design-specs/w2-dynamic-watcher-rebind.md`).
