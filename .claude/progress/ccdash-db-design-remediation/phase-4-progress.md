...
`sqlite3 data/ccdash_cache.db "SELECT COUNT(*) FROM telemetry_events WHERE event_type='p4_verification_seed';"` → 0 means the prune ran. Temporary `.env` verification overrides were removed at phase close (2026-06-03T23:25 ET); final operator env is exactly `CCDASH_RETENTION_PRUNE_ENABLED=true` (defaults: 24h interval, in-job VACUUM on). Live `last_run` was still unobserved at close — even after startup sync idled, the first tick did not fire within the watch window; root cause tracked under the liveness PRD. First scheduled production tick lands within 24h of next backend restart; verify via the seed-row one-liner or `/api/health/detail` → `retention.last_run`.
