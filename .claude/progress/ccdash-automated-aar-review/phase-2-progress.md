...
- id: T2-001
  ...
  completed: '2026-07-22T20:00:00Z'
  evidence:
  - code: backend/application/services/agent_queries/aar_review_enrichment.py
  verified_by:
...
- id: T2-002
  ...
  completed: '2026-07-22T20:00:00Z'
  evidence:
  - test: backend/tests/test_aar_review_enrichment.py
  verified_by:
  - task-completion-validator
- id: T2-003
  ...
  - task-completion-validator
- id: T2-004
  description: 'Sharpen context_ballooning with plan/task evidence: attach plan-declared
    effort/phase context to the flag''s evidence when linked task frontmatter is available;
    deterministic threshold logic unchanged from P1.'
  status: completed
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: T2-006
  description: 'Sharpen generic_agent_vs_specialist with assigned_to/assigned_model:
    ...
    assigned_to/assigned_model frontmatter (set-membership comparison). Static keyword->specialist
    lookup from P1 remains the fallback.'
  status: completed
  assigned_to:
  - python-backend-engineer
  dependencies:
...
- id: T2-008
  description: 'No-LLM compute-path assertion test (Hard Invariant #1): statically
    assert no LLM/model-client import (Anthropic SDK, OpenAI SDK, any Task/Agent dispatch
    helper) exists anywhere in aar_review.py or its enrichment-module dependency graph.'
  status: completed
  assigned_to:
  - python-backend-engineer
  dependencies:
  ...
  completed: '2026-07-22T20:00:00Z'
  evidence:
  - test: backend/tests/test_aar_review_no_llm_imports.py
  verified_by:
  - task-completion-validator
- id: T2-009
  description: 'Enrichment fixture suite: doc with a linked plan/task vs doc with
    no link; each flag''s sharpened-evidence path vs its P1 fallback path.'
  status: completed
  assigned_to:
  - python-backend-engineer
  dependencies:
...
notes: "Entry criteria: Phase 1 sealed (reconciled DTO + aar_reviews persistence live;\
  \ contract test + parity + direct-count green). Every enrichment comparison MUST\
  \ be set-membership, threshold, or static-ruleset lookup \u2014 any comparison requiring\
  \ semantic judgment is out of scope for this phase (Hard Invariant #1); descope\
  \ and record the finding. Phase 3's task-level scaffolding may start once T2-001's\
  \ evidence contract is frozen, but Phase 3 does not formally close until this phase\
  \ is sealed.\n"
progress: 100
---
