...
tasks:
- id: PCP-201
  description: Add a transport-neutral planning query layer for planning home, graph
    detail, feature planning context, and phase operations under the existing agent-query/service
    pattern.
  status: completed
  assigned_to:
  - backend-architect
  ...
  priority: high
- id: PCP-202
  description: Add or extend API endpoints for planning summary, graph/detail, feature
    planning context, and phase operations.
  status: completed
  assigned_to:
  - python-backend-engineer
  dependencies:
  ...
  priority: high
- id: PCP-203
  description: Add project and feature planning invalidation topics, plus any phase/worktree
...
blockers: []
notes:
- Phase 1 landed the derived planning graph, effective status, and phase batch model
  in `backend/services/feature_execution.py` / `backend/models.py` / `types.ts`; Phase
  2 exposes that via stable APIs and live topics.
- Commits land per batch as requested ("commit in batches").
success_criteria:
- id: SC-2.1
  description: Planning surfaces can bootstrap from stable APIs and remain fresh via
    live invalidation.
  status: completed
- id: SC-2.2
  ...
  status: completed
- id: SC-2.3
  description: No view needs to re-derive planning graph or mismatch logic from raw
    frontmatter.
  status: completed
files_modified: []
progress: 100
