...
5. **After the plan is approved**, emit the execution handoff and run the plan via `/dev:execute-plan`. Do not start execution here — this command is planning-only. The handoff names the orchestration-loop model per the `wave_plan.orchestrator_model` convention (plan-level default; `wave_plan.phases[].orchestrator_model` overrides a single phase — distinct from the delegate `phases[].model`). Emit it verbatim:

   > Execute: `/dev:execute-plan <plan-path>` — orchestrator: **<plan-default orchestrator_model>** (per-phase overrides: <Pn→model>, … listed only when any exist).

   Model IDs use the registry short form (`opus-4-8`, `sonnet-5`, `fable-5`). List the `(per-phase overrides: …)` clause only when at least one phase carries its own `orchestrator_model`; omit it otherwise.
...
- All Tier 2 steps and the mandatory decisions block apply — including the Tier 2 step 5 execution handoff. Tier 3 also executes via `/dev:execute-plan`, so emit the same `Execute:` handoff string; per-phase `orchestrator_model` overrides are more common here (Tier 3 / large-Tier-2, high-novelty or open-OQ phases).
