...
argument-hint: "[feature-text|REQ-ID|node_ID] [--worktree[=<path>]] [--no-worktree] [--plan-only] [--dry-run] [--budget=<tokens>] [--category=features|enhancements|refactors|harden-polish|infrastructure] [--max-points=N]"
allowed-tools: Read, Grep, Glob, Edit, Write, Task, EnterWorktree, ExitWorktree, Bash(git:*), Bash(pytest:*), Bash(pnpm:*), Bash(python:*), Bash(meatycapture:*), Bash(itt:*)
---
...
> **Git workflow.** Autopilot follows the [git worktree + PR protocol](../../skills/dev-execution/git-worktree-pr-protocol.md) §1a: the **parent branch** (HEAD at run start) is the PR base, work goes to a PR against the parent branch, and **squash-merge only happens on approval or an in-prompt override** (e.g. "auto-merge" / "merge when done"). Opus still orchestrates every git step.
>
> **Isolation: a verified worktree by default.** Autopilot runs in a git worktree it *enters* (`EnterWorktree`), so the run does not move HEAD in the checkout other sessions are standing in, and concurrent autopilot runs in one repo are possible. This is **default-on** and gated on a placement probe (Action 3); it falls back to a named branch in the session repo when the probe fails, and `--no-worktree` forces that lane outright.
>
> **Read this before "simplifying" Action 3.** Creating a worktree and *entering* one are different acts, and the difference is a shipped incident. On 2026-08-05 autopilot created `.claude/worktrees/autopilot-<slug>` with `git worktree add` while the session cwd never moved; the assigned branch received **zero** commits, both real commits landed on `main`, one was **pushed** — skipping the PR, review and squash gates — and the report read `status: complete`. A worktree the agents are not in reads as isolation without being it. Measured on Claude Code **2.1.224** (2026-08-07): after `EnterWorktree`, session Bash, `Agent` subagents **and background `Workflow` agents** all inherit the worktree cwd and branch, while the main checkout stays on its own branch. That measurement is why the lane is allowed; the probe is why it is trusted. Never swap `EnterWorktree` for `git worktree add`.
>
...
|---|---|---|
| `node_` + 26 alphanumerics | **IntentTree node** | Resolve via §1b below; bind `itt_node_id` |
...
- `--worktree` → force the worktree lane (already the default; use to override a project setting)
- `--worktree=<path>` → **reuse an existing worktree** at `<path>` instead of creating one
- `--no-worktree` → force the in-repo branch lane (§1a fallback)
...
### 1b. IntentTree node as the request (small nodes that don't warrant a planning session)

A node id is a legitimate autopilot input: the node **already carries** the request text, acceptance
criteria, and target repo, so the planning autopilot would otherwise synthesize is largely done.
This is the right entry point for a filed finding or a small `atomic_task` — not for a `work_package`
or anything with children.

```bash
itt --json node get "<node_id>" > /tmp/autopilot-node.json
python3 - <<'PY' < /tmp/autopilot-node.json
import json,sys
n=json.load(sys.stdin)
print("type:", n["type"], "| status:", n["status"], "| repo:", n.get("repo"), "| effort:", n.get("effort_size"))
print("children:", len(n.get("children") or []))
print("title:", n["title"])
print("ACs:", len(n.get("acceptance_criteria") or []))
PY
```

Then apply these gates **before** anything else:

| Node condition | Action |
|---|---|
| `type` is not `atomic_task` / `step` (i.e. it has structure) | **Stop** — route to `/plan:plan-feature`; autopilot is not a decomposer |
| `children` is non-empty | **Stop** — same reason |
| `status` is `in_progress` and `claimed_by_actor_id` is set | **Stop** — another agent holds it; say who |
| `status` is `completed` | **Stop** — report it as already done, do not re-run |
| `repo` names a repo other than this one | **Stop** — cross-repo autopilot is unsupported (the workflow's own `cross_repo_target` guard); re-run from that repo |
| `acceptance_criteria` is empty | Proceed, but say so: the reviewer gate will have nothing node-derived to check against |

Build `request` from the node: `title` as the headline, `description` as the body, and every
`acceptance_criteria` entry verbatim as a required AC (they are the node's contract — do not
paraphrase, and do not invent additional ones). Carry `effort_size` into your ceiling sanity check
(`l`/`xl` is a signal to stop and route to planning instead).

Claim the node before Action 2 so a concurrent loop does not take it:

```bash
itt node update "<node_id>" --status in_progress --meta autopilot_run=1
```

Prefer the MCP `claim_node` tool when atomicity matters — it 409s on an existing claim, where
`node update` will happily overwrite one. Bind `itt_node_id` for the completion step in Action 5, and
record the run branch on the node once Action 3 has named it
(`itt node update "<node_id>" --branch "${BRANCH}"`).

⚠️ **Do not mark the node complete here.** Completion belongs in Action 5, after the PR merges and
placement is verified — a node closed on a report that turns out to be fabricated is worse than an
open one.
...
How a dirty tree is handled **depends on the lane Action 3 will take**:

| Lane | Dirty-tree meaning |
|---|---|
| **worktree** (default) | Uncommitted changes **stay behind** in the main checkout — a fresh worktree does not carry them. Report them and continue. ⚠️ But if the dirty paths are files the request needs, the run will not see them: say so, and offer to commit them on the parent branch first (the worktree branches from `PARENT_TIP`, so only *committed* work is visible). |
| **in-repo** (`--no-worktree` / probe fallback) | **Not** warn-and-continue. `git switch -c` carries uncommitted changes onto the run branch, where the sprint commits them as part of the feature. Resolve them (commit or stash on the parent branch) before Action 3, or ask the user. Carrying someone else's work into a feature PR is not recoverable by re-reading the report. |

Either way, name the dirty paths in your pre-flight summary — in a shared checkout they may be another session's in-flight work, and `aos-git who` will say whether anyone else is here.

### 3. Set Up Isolation and the Run Branch
...
- **worktree lane (default)** — the session *enters* a worktree; agents inherit it (measured, then
  probed below). The main checkout is never touched.
- **in-repo lane** (`--no-worktree`, or probe failure) — the run branch is checked out in the session
  repo itself, moving HEAD for every other session in that checkout.

#### 3a. Resolve identity BEFORE entering (order matters)

Inside a worktree, `PARENT_BRANCH` is no longer readable from HEAD and `basename` of the toplevel is
the *worktree directory* name, not the repo. Capture all of this first:
...
# worktree dir name, which trips the workflow's cross_repo_target guard as a false positive.
...
#### 3b. Enter the worktree (skip entirely under `--no-worktree`)

Use the **`EnterWorktree` tool**, never `git worktree add`. Creating without entering is the
2026-08-05 incident, and it fails silently.

| Case | Call |
|---|---|
| new worktree (default) | `EnterWorktree` with `name: "autopilot-${SLUG}"` |
| `--worktree=<path>` | `EnterWorktree` with `path: "<path>"` |

For `--worktree=<path>`, verify the target first — the tool requires a worktree of this repo under
...
changes will be attributed to this feature. Report them and resolve before continuing.

#### 3c. Force the run branch onto the recorded parent tip

⚠️ **`worktree.baseRef` defaults to `fresh`, so a new worktree branches from `origin/<default-branch>`
— not from your `HEAD`.** Run autopilot from a feature branch, or from a `main` carrying unpushed
commits, and the worktree silently starts on the wrong base; the PR is then built against a parent
that is not the parent you recorded, and nothing in the report says so. Do not trust the base you
were handed:
...
test "$(git branch --show-current)" = "${BRANCH}" && echo "branch ok"   || echo "BRANCH MISMATCH"
test "${WORKTREE_ROOT}" != "${MAIN_ROOT}"         && echo "isolated ok" || echo "NOT IN A WORKTREE"
```

⚠️ `EnterWorktree name: "autopilot-${SLUG}"` creates its own branch named **`worktree-autopilot-${SLUG}`**
and checks that out. The `switch -C` above moves the worktree onto `${BRANCH}` (the name the workflow's
...
`git -C "${MAIN_ROOT}" branch -D "worktree-autopilot-${SLUG}"` (safe — it has no unique commits;
`-D` rather than `-d` because it was never merged anywhere). Do this **after** `ExitWorktree`.
...
Set `REPO_ROOT="${WORKTREE_ROOT}"` from here on: §4b and Action 5 run their git commands against the
tree the work actually landed in.
...
Verified while authoring this section: edits made from inside a worktree via absolute main-checkout
paths landed on `main`. So every git command from here on must use `-C "${REPO_ROOT}"` with
`REPO_ROOT` re-pointed as above, and the isolation you report is only as real as the paths you used.
This is the same class as the §4b instruction to always pass an explicit `-C`: the Bash tool resets
cwd between calls, so relative-vs-absolute is never something to leave implicit.

#### 3d. Probe placement — the gate that makes the isolation claim real

The measurement in the header is of **one harness version**. Doctrine that froze at one version is
precisely what produced the previous prohibition here, so verify rather than inherit. Skip this
entirely in the in-repo lane (there is nothing to verify — the session repo *is* the tree).

Check the cache first; it costs one cheap agent per Claude Code upgrade, not one per run:

```bash
CC_VERSION="$(claude --version 2>/dev/null | awk '{print $1}')"
CACHE="${MAIN_ROOT}/.claude/.autopilot-worktree-capability.json"
python3 - "$CACHE" "$CC_VERSION" <<'PY'
import json,sys,pathlib
p,v=pathlib.Path(sys.argv[1]),sys.argv[2]
try: d=json.loads(p.read_text())
except Exception: d={}
print("PROBE_NEEDED" if d.get("cc_version")!=v else f"CACHED:{d.get('verdict')}")
PY
```

`CACHED:inherits` → proceed. Anything else → run the probe: a **one-agent `Workflow`** (the real
dispatch path — an `Agent` subagent is not the same spawn path and is not a substitute) whose agent
runs exactly

```bash
echo "PWD=$(pwd) TOPLEVEL=$(git rev-parse --show-toplevel) BRANCH=$(git branch --show-current)"
```

and returns it verbatim. Require `TOPLEVEL == ${WORKTREE_ROOT}` **and** `BRANCH == ${BRANCH}`. Then
record the verdict:

```bash
python3 - "$CACHE" "$CC_VERSION" "<inherits|does_not_inherit>" <<'PY'
import json,sys,pathlib
p=pathlib.Path(sys.argv[1]); p.parent.mkdir(parents=True,exist_ok=True)
p.write_text(json.dumps({"cc_version":sys.argv[2],"verdict":sys.argv[3]},indent=2)+"\n")
print("recorded", sys.argv[3])
PY
```

**On `does_not_inherit` (or an inconclusive/failed probe), fall back — do not proceed in the
worktree.** The agents would commit somewhere the report will describe as isolated.

**Order matters here.** `${BRANCH}` is currently checked out *in the worktree*, and git refuses to
check the same branch out in a second tree (`fatal: ... is already used by worktree`). So tear the
worktree down **first**, then create the branch in the main checkout:
...
git -C "${MAIN_ROOT}" branch --show-current    # CONFIRM before invoking
```

Then set `REPO_ROOT="${MAIN_ROOT}"`, set `isolation: "branch_in_place"` in the args envelope, re-run
Action 2's dirty-tree check against the **in-repo** row of that table (it now applies, and it did
not before), and **tell the user the isolation degraded and why**. Silence here is the exact failure this section exists to
prevent: a run that reads as isolated without being it. Do not cache a `does_not_inherit` verdict as
if it were a user preference — it is a harness regression worth reporting.
...
```json
{
  ...
  "itt_node_id": "<node_* if resolved from IntentTree (§1b), else omit>",
  ...
  "session_repo": "<SESSION_REPO from Action 3a — from the SHARED git dir, never basename of the worktree>",
  "isolation": "<worktree|branch_in_place — which lane Action 3 actually ended in>",
  "worktree_path": "<WORKTREE_ROOT when isolation is worktree, else omit>",
  "ceiling": {
    "max_points": 13,
    "max_waves": 3,
    "max_phases": 8,
...
`isolation` and `worktree_path` are **descriptive, not instructive** — the workflow does not switch
trees (it cannot; it has no FS/shell). They exist so the report can state which lane ran instead of
leaving a reader to assume the safer one. `session_repo` must come from §3a's shared-git-dir
derivation: pass `basename "$(git rev-parse --show-toplevel)"` from inside a worktree and the
workflow halts with `cross_repo_target` against your own repo.
...
```bash
# 0. (worktree lane only) Did isolation actually hold?  Two independent facts:
git -C "${REPO_ROOT}" rev-parse --show-toplevel        # still ${WORKTREE_ROOT}, not ${MAIN_ROOT}
git -C "${MAIN_ROOT}" branch --show-current            # must STILL be ${PARENT_BRANCH}
git -C "${MAIN_ROOT}" log --oneline -3 "${PARENT_BRANCH}"   # and must NOT carry this run's commits

# 1. Did anything land on the branch we gave it?  This is the whole ballgame.
git -C "${REPO_ROOT}" log --oneline "${BASE_SHA}..${BRANCH}"
```

If the main checkout moved off `${PARENT_BRANCH}` or gained this run's commits, the isolation claim
is false regardless of what `run_placement` says — report that plainly, and treat the probe cache as
suspect (delete `.claude/.autopilot-worktree-capability.json` so the next run re-probes).
...
```json
{
  ...
  "run_placement": {
    ...
    "parent_moved": false,
    "isolation": "worktree|branch_in_place",
    "worktree_path": "<absolute path when isolation is worktree>"
  },
  "autopilot": {
    "tier": 0 | 1 | 2,
    "effort_points": N,
...
1. **Open a PR to the parent branch; squash-merge on approval/override** (protocol §5–6):
   ...
   **Then clean up according to the lane:**

   ```bash
   # worktree lane: leave the worktree (this also restores the session to ${MAIN_ROOT}).
   #   ExitWorktree action:"remove"  — after a successful squash-merge; the branch is merged and
   #                                  the local branch was deleted by --delete-branch.
   #   ExitWorktree action:"keep"    — if the PR is still open at the approval gate, or anything
   #                                  is unmerged. `remove` REFUSES on unmerged commits; do not
   #                                  reach for discard_changes:true to force it past that refusal
   #                                  without asking — that discards real work.
   # in-repo lane:
   git -C "${MAIN_ROOT}" switch "${PARENT_BRANCH}"   # restore the session's starting branch
   ```

   ⚠️ In the worktree lane, do **not** `git switch "${PARENT_BRANCH}"` inside the worktree — that
   checks the parent branch out in a second tree and is exactly the ground-shifting this lane avoids.
...
2b. **Close the IntentTree node** (if `itt_node_id` present) — only after §4b verified placement
   *and* the PR merged. Attach evidence in the same call so it lands in
   `meta.outcome.artifact_refs`:

   ```bash
   itt node complete "${ITT_NODE_ID}" \
     --evidence-kind pull_request \
     --evidence-ref "<PR URL>" \
     --evidence-label "autopilot: <slug>" \
     --evidence-delivery-class shipped
   itt node add-evidence "${ITT_NODE_ID}" --kind git_commit --ref "<squashed sha>" \
     --label "squash merge to ${PARENT_BRANCH}" --delivery-class shipped
   ```

   Keep `--label`/`--evidence-label` **short** — a long label 500s on the evidence endpoint, and the
   error names nothing. Use `--delivery-class branch_local` (never `shipped`) when the PR is still
   open at the approval gate; in that case do **not** call `node complete` at all — leave the node
   `in_progress` and say so. If the run produced nothing (`nothing_on_run_branch`) or the node was
   claimed via `claim_node`, release it back:
   `itt node update "${ITT_NODE_ID}" --status ready`.
...
- If the guard fired pre-dispatch: fix the branch and re-invoke. Nothing is lost.
  ```bash
  git -C "${REPO_ROOT}" switch -C "${BRANCH}" "${PARENT_TIP}"   # -C + PARENT_TIP: also repairs a wrong base
  git -C "${REPO_ROOT}" branch --show-current   # confirm before re-invoking
  ```
  ⚠️ In the worktree lane, `wrong_branch` has a second cause worth ruling out before you re-invoke:
  the tree may be correct while **`REPO_ROOT` points at the main checkout**, so you "fixed" a branch
  in the wrong tree. Confirm `git -C "${REPO_ROOT}" rev-parse --show-toplevel` equals
  `${WORKTREE_ROOT}` first. Absolute paths ignore the session's cwd — that is how this class of
  mistake happens.
...
- Clean up the run branch (it holds nothing — the workflow escalated before executing):
  ```bash
  # worktree lane: ExitWorktree action:"remove"  (it is empty; this also removes the branch)
  # in-repo lane:
  git -C "${MAIN_ROOT}" switch "${PARENT_BRANCH}"
  git -C "${MAIN_ROOT}" branch -D "${BRANCH}"   # refuses if commits exist; then keep it and inspect
  ```
  If `ExitWorktree action:"remove"` refuses, commits **do** exist — the escalation claim and the
  branch disagree. Keep the worktree and inspect before believing either.
...
| Flag | Default | Effect |
|---|---|---|
| `--worktree` | **on** | Run in a worktree the session enters (Action 3). Explicit form for overriding a project default |
| `--worktree=<path>` | — | Reuse the existing worktree at `<path>` instead of creating one; must be a registered worktree of this repo |
| `--no-worktree` | false | Force the in-repo branch lane — moves HEAD in the shared checkout; use only when worktrees are impractical |
...
- Isolation + run-branch setup (Action 3) is the only Opus direct action before invocation. Its
  placement probe costs one cheap agent **per Claude Code version**, not per run — check the cache
  before spawning it.
...
- [ ] IntentTree node completed with evidence, or explicitly left `in_progress` with the reason stated (if `itt_node_id` present)
- [ ] `BASE_SHA == PARENT_TIP` — the run was based on the parent you recorded, not on `origin/<default>`
- [ ] **worktree lane:** main checkout still on `${PARENT_BRANCH}` and free of this run's commits; session returned via `ExitWorktree` (removed after merge, kept if unmerged)
- [ ] **in-repo lane:** session restored to `${PARENT_BRANCH}`; run branch deleted after merge (no orphan branches)
- [ ] If isolation degraded to `branch_in_place`, that was stated to the user — never left implicit
