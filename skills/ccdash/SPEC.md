...
> ⚠️ **Recovered artifact.** This SPEC existed only in the `MeatySkills/skills/ccdash/` lineage and was
> absent from the `skillmeat/.claude/skills/ccdash/` one; it was recovered during the 2026-08-14 lineage
> reconciliation that moved this skill's upstream to the CCDash repo. Its own `skill_version: 2.2.0` and
> `updated: 2026-05-29` are the values it was written with and have deliberately **not** been rewritten —
> the skill is now at 2.3, so treat this file's inventories as a 2026-05-29 snapshot, not current. See
> `CHANGELOG.md` for what changed since, and `SKILL.md`'s Confidence Anchor for the surfaces verified at
> CCDash `670bcc9`.
